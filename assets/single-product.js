class VariantRadiosProductSingle extends HTMLElement {
    constructor() {
        super();
        this.querySelectorAll('.tps__option-swatch').forEach(
    ...
    onVariantChange(event) {
        ...
        for (var item of target.closest('fieldset').querySelectorAll('.tps__option-swatch')) {
            item.classList.remove('active');
        }
        target.classList.toggle('active');
        target.closest('fieldset').querySelector('.swatch-selected-value').textContent = value;
        this.options = Array.from(this.querySelectorAll('.tps__option-swatch.active'), (select) => select.getAttribute('data-value'));
        this.updateMasterId();
    ...
    renderProductInfo() {
        ...
        const product = form.closest('.tps__product-single');
        ...
        if (product.querySelector('.tps__availability-value')) {
            product.querySelector('.tps__availability-value').textContent = availability;
        }
        if (this.currentVariant.available && qty < 1) {
            pre_order = true;
        } else if (!this.currentVariant.available) {
        ...
        const product_label = product.querySelector('.tps__product-label');
        if (product_label) {
            product_label.remove();
        }
        if (sale || pre_order || sold_out) {
            var element = document.createElement('div');
            element.classList.add('tps__product-label', 'fs-12', 'pointer-events-none', 'absolute');
            product.querySelector('.tps__product-details').insertBefore(element, product.querySelector('.tps__product-details').children[0]);
            const label = product.querySelector('.tps__product-label');
            if (sold_out) {
        ...
        if (this.currentVariant.featured_media) {
            setTimeout(() => {
                product.querySelector('.tps__product-main-img img').removeAttribute('srcset');
                product.querySelector('.tps__product-main-img img').setAttribute('src', this.currentVariant.featured_media.preview_image.src);
            }, 500);
        }

        product.querySelector('.tps__product-sku-value').textContent = this.currentVariant.sku;
        ...
        tps__price.querySelector('.price__regular .price').classList.remove('special-price');
        if (compare_at_price && compare_at_price > price) {
            ...
            tps__price.querySelector('.price__regular .price').classList.add('special-price');
        } else if (!this.currentVariant.available) {
    ...
    updateVariantStatuses(){
        ...
        inputWrappers.forEach((option, index) => {
            if (index === 0) return;
            const optionInputs = [...option.querySelectorAll('.tps__option-swatch')];
    ...
    eventProductGroupAction() {
        document.querySelectorAll('#tps__product-group .product-variant-option').forEach(select => {
            select.addEventListener("change", (event) => {
                var target = event.target;
                var 
                    image = target.options[target.selectedIndex].getAttribute('data-image'),
                    pro_handle = target.getAttribute('data-handle')
                var img = target.closest('#tps__product-group').querySelector('.product-group-image-item.'+pro_handle+'').querySelector('img');
                if ( img ) {
    ...
    submitProductGroup(event) {
        ...
        const form = document.getElementById('tps__product-group-form');
    ...
    actionDropdownSwatches(){
        this.querySelectorAll('[data-swatches-value]').forEach((items) => {
            items.addEventListener('click' , (e) => {
               ...
               if (!target.closest(".tps__color-dropdown").classList.contains("isClicked")) {
                 for (var item of document.querySelectorAll('.tps__color-dropdown')){
        ...
        this.querySelectorAll('.tps__product-color-swatches-dropdown').forEach((swatches) => {
            swatches.addEventListener('click', (e) => {
                const target = e.currentTarget;
                const valueSwatch = target.dataset.value;
                const container = target.closest('.tps__color-dropdown');
                container.querySelector('.tps__color-dropdown-action .tps__color-dropdown-value').innerHTML = valueSwatch;
                target.closest('.tps__color-dropdown').classList.remove('isClicked')
            })
        })
      }
    ...
    return {
        ...
        eventProductGroup: function() {
            ...
            .then(async (responseText) => {
                ...
                document.getElementById('tps__product-group').innerHTML = html.querySelector('.tps__product-group').innerHTML;
            }).catch((e) => {
                throw error;
            }).finally(() => {
