...
var TpsRVPageShopify = (function() {
    return {
        initRVItems: function() {
            if (window.location.search.indexOf("page=") == -1) {
                const rv_items = JSON.parse(localStorage.getItem('tps__recently-viewed-products'));
                this.init(rv_items);
            }
        },
        ...
        init: function (rv_items) {
            const _this = this
            const rv_div = document.querySelector('.tps__rv-page-main');
            const div_no_product = rv_div.querySelector('.tps__rv-no-product-js');
            ...
            if (rv_items === null || rv_items.length === 0) {
                ...
                .then((responseText) => {
                    ...
                    const er = html.querySelector(".tps__rv-page-main > .row")
                    if(rv_items.length !== 0 && er){
                        rv_div.innerHTML = html.querySelector(".tps__rv-page-main").innerHTML;
        ...
        clearAll: function(){
            const rv_div = document.querySelector('.tps__rv-page-main');
            if (rv_div) {
                if (!rv_div.querySelector(".tps__clear-all-rvp")) return;
                rv_div.querySelector(".tps__clear-all-rvp").addEventListener("click", function () {
                    rv_div.querySelector(".tps__clear-all-rvp").classList.add("d-none");
                    localStorage.removeItem('tps__recently-viewed-products');
                    TpsRVPageShopify.initRVItems();
                })
            }
        ...
        skeletonFunction: function (timer) {
            window.setTimeout(function() {
                ...
                if (this.document.querySelector('.tps__rv-page-section-inner')) {
                    this.document.querySelector('.tps__rv-page-section-inner').classList.remove('d-none');
                };
            },timer);
        }
      }
